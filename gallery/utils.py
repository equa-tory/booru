...
def make_video_thumb(src_path, max_size=360):
    """Extract first usable frame from video using ffmpeg."""
    thumb_path = _thumb_path_for(src_path)
    result = None
...
def make_thumb(src_path, max_size=360):
    if is_video(src_path):
        return make_video_thumb(src_path, max_size)
    if is_pdf(src_path):
...
def ingest_photo(path, post, order=0):
    ...
    ph    = compute_phash(path) if not vid and not pdf else ''
...
def scan_inbox():
    ...
    if os.path.isdir(multi_dir):
        for folder_name in sorted(os.listdir(multi_dir)):
